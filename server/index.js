...
app.get('/profiles/:id', authMiddleware, (req, res) => {
  const id = req.params.id;
  const callerRoles = Array.isArray(req.user.roles) ? req.user.roles : [req.user.role];
  const isAdmin = callerRoles.some(r => ['admin', 'superadmin'].includes(r));
  // Users can only fetch their own profile unless they are admin
  if (req.user.id !== id && !isAdmin) return res.status(403).json({ error: 'forbidden' });
...
app.get('/user_roles', authMiddleware, (req, res) => {
  const user_id = req.query.user_id;
  if (!user_id) return res.status(400).json({ error: 'user_id required' });
  const callerRoles = Array.isArray(req.user.roles) ? req.user.roles : [req.user.role];
  const isAdmin = callerRoles.some(r => ['admin', 'superadmin'].includes(r));
  // Users can only fetch their own roles unless they are admin
  if (req.user.id !== user_id && !isAdmin) return res.status(403).json({ error: 'forbidden' });
...
async function extractFileText(fileUrl, fileName) {
  try {
    ...
    if (nameLower.endsWith('.pdf')) {
      ...
      return r.text?.slice(0, 15000) || null;
    }
    if (nameLower.endsWith('.docx') || nameLower.endsWith('.doc')) {
      const r = await mammoth.extractRawText({ buffer: buf });
      return r.value?.slice(0, 15000) || null;
    }
...
app.post('/chat', authMiddleware, async (req, res) => {
  ...
  const roles = db.prepare('SELECT role FROM user_roles WHERE user_id = ?').all(userId).map(r => r.role);
  const isStudent = roles.length === 0 || (roles.every(r => r === 'student'));
  if (isStudent) return res.status(403).json({ error: 'forbidden' });

  const { messages = [], thesisId, contextType = 'none' } = req.body;
  if (!Array.isArray(messages)) return res.status(400).json({ error: 'messages must be an array' });

  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) return res.status(503).json({ error: 'GioBot no está configurado (falta ANTHROPIC_API_KEY)' });

  let contextData = '';
  try {
    if (thesisId && contextType === 'single') {
      const thesis = db.prepare(`
        SELECT t.id, t.title, t.status, t.defense_date, t.defense_location, t.defense_info,
               t.final_weighted_override, t.created_at
        FROM theses t WHERE t.id = ?
      `).get(thesisId);
      if (thesis) {
        const students = db.prepare(`
          SELECT u.full_name, u.student_code, u.cedula, u.institutional_email
        ...
        const thesisObj = {
          ...thesis,
          defense_date: thesis.defense_date ? new Date(thesis.defense_date * 1000).toLocaleString('es-CO') : null,
      ...
      let thesisIds;
      if (isAdmin) {
        thesisIds = db.prepare(`SELECT id FROM theses ORDER BY created_at DESC LIMIT 100`).all().map(r => r.id);
      } else {
        const directedIds = db.prepare(`SELECT thesis_id as id FROM thesis_directors WHERE user_id = ?`).all(userId).map(r => r.id);
        const assignedIds = db.prepare(`SELECT thesis_id as id FROM thesis_evaluators WHERE evaluator_id = ?`).all(userId).map(r => r.id);
        thesisIds = [...new Set([...directedIds, ...assignedIds])];
      }

      const theses = thesisIds.map(id => {
        const t = db.prepare(`SELECT id, title, status, defense_date FROM theses WHERE id = ?`).get(id);
        if (!t) return null;
        const students = db.prepare(`
          SELECT u.full_name, u.student_code, u.cedula, u.institutional_email, u.cvlac
          FROM thesis_students ts JOIN users u ON ts.student_id = u.id WHERE ts.thesis_id = ?
        `).all(id);
        const directors = db.prepare(`
          SELECT u.full_name, u.institutional_email, u.email FROM thesis_directors td JOIN users u ON td.user_id = u.id WHERE td.thesis_id = ?
  ...
  const systemPrompt = `Eres GioBot, el asistente IA del sistema SisTesis de la Universidad de San Buenaventura Cali (USB Cali). Ayudas a directores de tesis, evaluadores y administradores a consultar información sobre proyectos de grado Y a entender cómo usar la plataforma.
El usuario actual tiene rol(es): ${roles.join(', ')}.
...
--- ROLES Y ACCESOS ---
- Estudiante: registra su proyecto, sube documentos, ve su línea de tiempo.
- Evaluador: puede ser evaluador de comité (califica con rúbrica) y/o director (dirige y avala proyectos de sus estudiantes).
- Administrador: gestiona todos los proyectos, asigna evaluadores, configura la plataforma. También puede ser director de programa y tener sus propios estudiantes.
- Superadministrador: todo lo del administrador más gestión de usuarios y configuración SMTP.

--- FLUJO COMPLETO DE UN PROYECTO DE GRADO ---
1. El estudiante registra su proyecto de grado (título, descripción, programa).
2. El proyecto queda en estado "submitted" (enviado).
3. El administrador/director revisa los requisitos previos.
4. Si hay observaciones, el admin puede devolver el proyecto (estado "returned") con comentarios.
5. Cuando cumple los requisitos, el admin marca los ítems de revisión y el proyecto pasa a "approved".
6. El admin asigna evaluadores al proyecto (estado "assigned").
7. Los evaluadores califican con rúbrica (estado "evaluated").
8. El estudiante defiende su proyecto (estado "defended").
9. Se genera acta de grado y firmas digitales (estado "finalized").

--- CÓMO ASIGNAR EVALUADORES A UN PROYECTO ---
Paso 1: Ir al menú "Proyectos" en la barra lateral izquierda.
Paso 2: Buscar el proyecto que deseas gestionar y hacer clic en él para abrir el detalle.
Paso 3: Verificar que el proyecto cumple los requisitos previos. En la sección de revisión verás una lista de ítems que deben estar marcados:
  - Revisión (el documento ha sido revisado)
  - Aval director(es) (el director ha dado su aval)
  - Compilatio < 20% (el índice de similitud es menor al 20%)
  Si algún ítem no está marcado, debes marcarlo haciendo clic en el checkbox correspondiente.
Paso 4: Una vez marcados TODOS los ítems de revisión, el sistema habilitará la opción de asignar evaluadores. Busca la sección "Evaluadores" o "Asignar evaluadores" dentro del detalle del proyecto.
Paso 5: En la sección de evaluadores, haz clic en "Agregar evaluador" o en el botón de asignación.
Paso 6: Selecciona el evaluador de la lista de evaluadores disponibles. Puedes asignar uno o más evaluadores.
Paso 7: Confirma la asignación. El proyecto cambiará automáticamente al estado "assigned" (evaluador asignado) y el evaluador recibirá una notificación por correo electrónico.

--- CÓMO DEVOLVER UN PROYECTO AL ESTUDIANTE ---
Paso 1: Abrir el detalle del proyecto desde "Proyectos".
Paso 2: Buscar el botón "Devolver proyecto" o "Retornar".
Paso 3: Escribir las observaciones o comentarios explicando qué debe corregir el estudiante.
Paso 4: Confirmar. El proyecto vuelve al estado "returned" y el estudiante recibe notificación.

--- CÓMO CALIFICAR UN PROYECTO (EVALUADOR) ---
Paso 1: Ir al menú "Proyectos asignados" en la barra lateral.
Paso 2: Seleccionar el proyecto que debes evaluar.
Paso 3: Hacer clic en "Evaluar" o "Calificar con rúbrica".
Paso 4: Se abrirá la rúbrica de evaluación con todas las secciones y criterios.
Paso 5: Para cada criterio, selecciona el nivel de desempeño (Deficiente, Aceptable, Bueno, Excelente) o ingresa la calificación numérica sobre 5.0.
Paso 6: Puedes agregar comentarios por sección.
Paso 7: Al finalizar, haz clic en "Guardar" para guardar sin enviar, o "Enviar calificación" para enviar la evaluación definitiva.
Paso 8: Una vez enviada, la calificación queda registrada y no puede modificarse.

--- CÓMO VER MIS ESTUDIANTES (DIRECTOR) ---
- Como evaluador-director: ir al menú "Mis estudiantes" en la barra lateral. Allí verás todos los proyectos que diriges.
- Como administrador-director: ir al menú "Mis Estudiantes" en la barra lateral de admin.
- Desde esa vista puedes ver el estado de cada proyecto, los documentos subidos y el avance.

--- CÓMO GESTIONAR EVALUADORES (ADMIN) ---
Paso 1: Ir al menú "Evaluadores" en la barra lateral.
Paso 2: Verás la lista de todos los evaluadores registrados con sus proyectos asignados.
Paso 3: Para agregar un nuevo evaluador, haz clic en "Agregar evaluador" e ingresa sus datos.
Paso 4: Para enviarle sus credenciales de acceso, busca el botón "Enviar acceso" junto al evaluador.

--- CÓMO GESTIONAR PROGRAMAS ACADÉMICOS (ADMIN) ---
Paso 1: Ir al menú "Programas" en la barra lateral.
Paso 2: Puedes crear, editar o desactivar programas académicos.
Paso 3: Cada programa tiene un nombre, descripción y puede tener ventanas de recepción de proyectos.
Paso 4: Las ventanas de recepción definen los períodos en que los estudiantes pueden registrar proyectos.

--- CÓMO CONFIGURAR RÚBRICAS (ADMIN) ---
Paso 1: Ir al menú "Rúbricas" en la barra lateral.
Paso 2: Puedes crear rúbricas con secciones y criterios de evaluación.
Paso 3: Cada criterio tiene niveles de desempeño con puntajes.
Paso 4: Las rúbricas se asignan a los evaluadores para calificar proyectos.

--- CÓMO VER ESTADÍSTICAS Y REPORTES (ADMIN) ---
- En el "Panel" (dashboard) verás estadísticas generales: total de proyectos, distribución por estado, calificaciones promedio.
- Desde el detalle de un proyecto puedes descargar reportes en PDF.
- Desde la lista de proyectos puedes exportar datos en Excel/ZIP con las rúbricas.

--- CÓMO FUNCIONA LA FIRMA DIGITAL DE ACTAS ---
Paso 1: Cuando un proyecto llega a estado "defended", se puede generar el acta de grado.
Paso 2: El sistema envía un enlace de firma a cada firmante (director, evaluadores, director de programa).
Paso 3: Cada firmante accede al enlace, revisa el acta y puede firmar dibujando su firma o subiendo una imagen.
Paso 4: Cuando todos han firmado, el acta queda finalizada y el proyecto pasa a estado "finalized".

--- ESTADOS DE LOS PROYECTOS ---
- submitted: El estudiante registró el proyecto, espera revisión del admin.
- under_review: El admin está revisando el proyecto.
- returned: El admin devolvió el proyecto al estudiante con observaciones.
- approved: El proyecto cumple requisitos, listo para asignar evaluadores.
- assigned: Se asignaron evaluadores, esperando evaluación.
- evaluated: Los evaluadores calificaron el proyecto.
- defended: El estudiante presentó su defensa.
- finalized: Proceso completo, acta firmada.

--- NOTIFICACIONES ---
- El sistema envía correos automáticos en eventos clave: asignación de evaluadores, devolución de proyectos, nuevos documentos, etc.
- Los administradores pueden configurar reglas de notificación personalizadas en "Reglas de notificación".
- GioBot (este asistente) también te avisa de tareas pendientes cuando abres el chat.

--- PREGUNTAS FRECUENTES ---
P: No puedo asignar evaluadores, ¿por qué?
R: Debes marcar TODOS los ítems de revisión primero (Revisión, Aval director, Compilatio < 20%). Solo cuando todos estén marcados se habilita la asignación.

P: El evaluador no recibe el correo de notificación.
R: Verifica la configuración SMTP en "Configuración SMTP" (solo superadmin). Asegúrate de que las credenciales sean correctas y el servidor SMTP esté activo.

P: ¿Cómo cambio la contraseña de un usuario?
R: Solo el superadministrador puede gestionar usuarios desde el menú "Usuarios". Desde allí puede cambiar contraseñas o enviar accesos.

P: ¿Puedo editar una calificación ya enviada?
R: No. Una vez que el evaluador envía la calificación definitiva, no puede modificarse. Contacta al administrador si hay un error.
=== FIN DOCUMENTACIÓN ===
${contextData}`;
  ...
  try {
    ...
    res.status(500).json({ error: 'Error al consultar GioBot: ' + (err.message || 'unknown') });
  }
});
...
app.get('/chat/alerts', authMiddleware, (req, res) => {
  ...
  try {
    const isEvaluator = roles.some(r => r === 'evaluator');
    const isAdmin = roles.some(r => r === 'admin' || r === 'superadmin');

    if (isEvaluator) {
      // Tesis asignadas como evaluador sin evaluación completada
      const pendingEval = db.prepare(`
        SELECT t.title FROM thesis_evaluators te
        JOIN theses t ON t.id = te.thesis_id
        LEFT JOIN evaluations e ON e.thesis_evaluator_id = te.id
        WHERE te.evaluator_id = ? AND te.role = 'evaluator'
          AND (e.id IS NULL OR e.final_score IS NULL)
          AND t.status NOT IN ('finalized')
      `).all(userId);
      if (pendingEval.length > 0) {
        alerts.push(`Tienes ${pendingEval.length} tesis pendiente(s) de evaluar: ${pendingEval.map(t => t.title).join(', ')}.`);
      }

      // Tesis dirigidas con documentos nuevos (últimas 48h)
      const recentDocs = db.prepare(`
        SELECT COUNT(*) as cnt FROM thesis_files tf
        JOIN theses t ON t.id = tf.thesis_id
        JOIN thesis_evaluators te ON te.thesis_id = t.id
        WHERE te.evaluator_id = ? AND te.role = 'director'
          AND tf.uploaded_at >= datetime('now', '-48 hours')
      `).get(userId);
      if (recentDocs && recentDocs.cnt > 0) {
        alerts.push(`${recentDocs.cnt} documento(s) nuevo(s) subido(s) en las últimas 48 horas en tus tesis dirigidas.`);
      }
    }

    if (isAdmin) {
      // Tesis enviadas sin asignar evaluador
      const unassigned = db.prepare(`
        SELECT COUNT(*) as cnt FROM theses t
        LEFT JOIN thesis_evaluators te ON te.thesis_id = t.id AND te.role = 'evaluator'
        WHERE t.status IN ('submitted','under_review') AND te.id IS NULL
      `).get();
      if (unassigned && unassigned.cnt > 0) {
        alerts.push(`${unassigned.cnt} tesis enviada(s) sin evaluador asignado.`);
      }

      // Tesis asignadas con evaluación pendiente hace más de 7 días
      const stalePending = db.prepare(`
        SELECT COUNT(*) as cnt FROM theses t
        JOIN thesis_evaluators te ON te.thesis_id = t.id AND te.role = 'evaluator'
        LEFT JOIN evaluations e ON e.thesis_evaluator_id = te.id
        WHERE t.status = 'assigned'
          AND (e.id IS NULL OR e.final_score IS NULL)
          AND t.updated_at <= datetime('now', '-7 days')
      `).get();
      if (stalePending && stalePending.cnt > 0) {
        alerts.push(`${stalePending.cnt} tesis asignada(s) sin evaluación completada desde hace más de 7 días.`);
      }
    }
  } catch (err) {
    console.error('GioBot alerts error:', err);
  }
